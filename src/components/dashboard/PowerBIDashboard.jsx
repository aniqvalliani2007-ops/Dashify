...
export const PowerBIDashboard = ({ fileRows, headers, transformConfig }) => {
  ...
  return (
    <div className="space-y-10 fade-in" key={refreshKey}>
      {/* Header */}
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
        <div className="flex items-start gap-5">
          <div className="p-4 bg-blue-600 text-white" style={{borderRadius: '2px'}}>
            <LayoutDashboard size={28} />
          </div>
          <div>
            <h2 className="text-3xl font-semibold text-gray-900 mb-2" style={{letterSpacing: '-0.03em'}}>Power BI Dashboard</h2>
            <p className="text-base text-gray-600">Auto-generated intelligent insights</p>
          </div>
        </div>
        ...
        {keyMetrics.map((metric, idx) => (
          ...
          >
            <div className="flex items-start justify-between mb-5">
              <div className={`p-3 bg-${metric.color}-50 text-${metric.color}-600`} style={{borderRadius: '2px'}}>
                <metric.icon size={22} />
              </div>
              <Maximize2 size={16} className="text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        {dashboardData.map((viz, idx) => (
          <div key={idx} className="chart-container flex flex-col" style={{minHeight: '460px'}}>
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
              ...
              <div className={`p-2.5 ${
                ...
                {viz.type === 'bar' && <BarChart3 size={18} />}
                {viz.type === 'line' && <Activity size={18} />}
                {viz.type === 'pie' && <PieChartIcon size={18} />}
              </div>
            </div>
      ...
      {/* Data Insights */}
      <div className="glass-card p-8 border border-blue-100 bg-gradient-to-r from-blue-50/50 to-blue-100/50" style={{borderRadius: '2px'}}>
        <h3 className="text-lg font-semibold text-blue-900 mb-5 flex items-center gap-3">
          <TrendingUp size={22} />
          Quick Insights
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-sm text-blue-800">
